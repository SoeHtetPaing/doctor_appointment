...
<?php
    ...
    <div class="container">
        ...
        <?php
                ...
                $sqlmain= "select * from schedule inner join doctor on schedule.did=doctor.did where schedule.sdate>='$today'  order by schedule.sdate asc";
                $sqlpt1="";
                $insertkey="";
                $q='';
                $searchtype="All";
                        ...
                        if(!empty($_POST["search"])){
                            ...
                            $sqlmain= "select * from schedule inner join doctor on schedule.did=doctor.did where schedule.sdate>='$today' and (doctor.dname='$keyword' or doctor.dname like '$keyword%' or doctor.dname like '%$keyword' or doctor.dname like '%$keyword%' or schedule.title='$keyword' or schedule.title like '$keyword%' or schedule.title like '%$keyword' or schedule.title like '%$keyword%' or schedule.sdate like '$keyword%' or schedule.sdate like '%$keyword' or schedule.sdate like '%$keyword%' or schedule.sdate='$keyword' )  order by schedule.sdate asc";
                            //echo $sqlmain;
                            $insertkey=$keyword;
                            ...
                            <?php
                                ...
                                for ( $x=0; $x<($result->num_rows);$x++){
                                    ...
                                    for($q=0;$q<3;$q++){
                                        ...
                                        $title=$row["title"];
                                        $docname=$row["dname"];
                                        $scheduledate=$row["sdate"];
                                        $scheduletime=$row["stime"];
                                        $speciality=$row["specialties"];
                                        $speciality_name = selectSpecialityName($database, $speciality);
                                        $speciality_name = $speciality_name["sname"];
                                        ...
                                        <td style="width: 25%;">
                                                <div  class="dashboard-items search-items"  >
                                                    ...
                                                    <div style="width:100%; position: relative;">
                                                            ...
                                                            <div class="h3-search" style="font-weight: bold;">
                                                                '.substr($speciality_name,0,100).'
                                                            </div>
                                                            <div class="h4-search">
                                                                '.$scheduledate.'<br>Starts: <b>@'.substr($scheduletime,0,5).'</b> (24h)
                                                            </div>
                                                            <div style="width: 70px; height: 70px; background-image: url(../img/icons/speciality-'.$speciality.'.png); background-size: cover; background-position: center; border-radius: 50%; position: absolute; right: 0; top: 25%;">
                                                            </div>
                                                            <br>
